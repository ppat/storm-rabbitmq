 ...
 * Should be used when Storm's guaranteed message processing is not needed.
 * Messages will be removed from RabbitMQ queue as soon as it's delivered to
 * storm and will not be retried on any errors during processing.
 *
...
public class UnanchoredRabbitMQSpout extends RabbitMQSpout {
	...
	protected RabbitMQConsumer loadConsumer(Declarator declarator, ErrorReporter reporter, ConsumerConfig config) {
		return new UnanchoredConsumer(config.getConnectionConfig(), config.getPrefetchCount(), config.getQueueName(), config.isRequeueOnFail(), declarator,
				reporter);
	}
