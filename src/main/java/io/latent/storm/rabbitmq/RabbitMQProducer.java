...
public class RabbitMQProducer implements Serializable {
  ...
  public void send(Message message, String routingKey) {
    sendMessageWhenNotBlocked(message, routingKey);
  }

  public void sendMessageWhenNotBlocked(Message message, String routingKey) {
    while (true) {
      if (blocked) {
        try {
          Thread.sleep(100);
  ...
  private void sendMessageActual(Message message, String routingKey) {
    ...
    try {
      AMQP.BasicProperties properties = new AMQP.BasicProperties.Builder().contentType(producerConfig.getContentType())
          .contentEncoding(producerConfig.getContentEncoding()).deliveryMode((producerConfig.isPersistent()) ? 2 : 1).headers(getHeaders(message)).build();
